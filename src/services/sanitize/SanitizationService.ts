...
const SAFE_IMAGES: NonEmptyList<AssetUrl> = NonEmptyList(
  image_01,
  List.from([
    image_02,
    image_03,
...
const SAFE_PHRASES: NonEmptyList<string> = NonEmptyList(
...
export const imageMappings = memoize(
  (key) => randomPickNonEmptyList(SAFE_IMAGES),
  { max: 100, length: 1 }
);

export const phraseMappings = memoize(
  (key) => randomPickNonEmptyList(SAFE_PHRASES),
  { max: 100, length: 1 }
);
