...
export const searchVideos = (
  ...
  axiosClient
    .get(
      `/videos/search?page-number=${pageNumber}&page-size=${pageSize}${searchTerm.fold(
        String()
...
export const fetchVideoSnapshots = (videoId: string): Promise<Snapshot[]> =>
  axiosClient
    .get(`/videos/id/${videoId}/snapshots`)
...
export const analyze = (url: string): Promise<VideoAnalysisResult> =>
  axiosClient
    ...
    .then(({ data }) => parseVideoAnalysisResult(data));
...
export const updateVideoTitle = (
  ...
  axiosClient
    .patch(`/videos/id/${videoId}/metadata`, { title })
