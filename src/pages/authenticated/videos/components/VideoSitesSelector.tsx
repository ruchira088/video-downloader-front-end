...
export default (props: {videoSites: Maybe<NonEmptyList<string>>, onChange: (values: Maybe<NonEmptyList<string>>) => void, className?: string}): JSX.Element => {
  const [availableVideoSites, setAvailableVideoSites] = useState<string[]>([])

  useEffect(() => {
    videoServiceSummary()
      .then(summary => setAvailableVideoSites(summary.sites))
  }, [])
  ...
  return (
    <Select
      ...
      value={availableVideoSites.length === 0 ? [] : props.videoSites.map(nonEmptyList => nonEmptyList.toArray()).getOrElse([])}
