...
export default () => {
  const [serviceInformation, setServiceInformation] = useState<
    Maybe<BackendServiceInformation>
  >(None());
  const [frontendInformation, setFrontendInformation] = useState(
    frontendServiceInformation(process.env)
  );

  const fetchServerInformation = () =>
    backendServiceInformation().then((information) =>
      setServiceInformation(Some(information))
    );
    ...
    <div className="service-information">
      <div>
        <ServiceInformationItem
          label="API URL"
          value={Some(configuration.apiService)}
        />
