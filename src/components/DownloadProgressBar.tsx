...
export default (progressValue: ProgressValue) => {
  const percentage: number = Number(
    ((progressValue.currentValue / progressValue.completeValue) * 100).toFixed(
      2
    )
  );
  ...
  return (
    <div className="download-progress-bar">
      ...
      <div>{percentage} %</div>
      <div>
        {humanReadableSize(progressValue.currentValue)} /{" "}
        {humanReadableSize(progressValue.completeValue)}
      </div>
